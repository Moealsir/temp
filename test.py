    ...
    try:
        ...
        for user in credentials_data['users']:
            if user['number'] == user_id:
                return {
                    "installed": user['gmail_credentials']  # Change 'web' to 'installed'
                }
...
def load_credentials(user):
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            creds = flow.run_local_server(port=user.get("port", 0))
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=5001, debug=True, ssl_context=(ssl_cert_path, ssl_key_path))
